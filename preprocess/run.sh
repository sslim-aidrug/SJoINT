...
#   bash preprocess/run.sh moleculeace   # Tensorize MoleculeACE (canonical split)
#
...
case "${1:-}" in
  pretrain)
    INPUT_TXT="${INPUT_TXT:-data/ZINC250K/jtnn_zinc250k.txt}"
    OUT_PATH="${OUT_PATH:-data/ZINC250K/data}"
    VOCAB_PATH="${VOCAB_PATH:-data/ZINC250K/vocab.json}"
    WORKERS="${WORKERS:-60}"
  ...
  moleculenet|finetune)
    MODE="${MODE:-random}"
    CSV_DIR="${CSV_DIR:-data/MoleculeNet/raw}"
    VOCAB_PATH="${VOCAB_PATH:-data/ZINC250K/vocab.json}"
    SEEDS="${SEEDS:-1 2 3 42 43 44 123 456 789 1024}"

    if [ "$MODE" = "both" ]; then
        OUT_DIR="${OUT_DIR:-data/MoleculeNet/split}"
    elif [ "$MODE" = "scaffold" ]; then
        OUT_DIR="${OUT_DIR:-data/MoleculeNet/scaffold_split}"
    else
        OUT_DIR="${OUT_DIR:-data/MoleculeNet/random_split}"
    fi
  ...
  moleculeace)
    CSV_DIR="${CSV_DIR:-data/MoleculeACE/raw}"
    OUT_DIR="${OUT_DIR:-data/MoleculeACE/processed}"
    VOCAB_PATH="${VOCAB_PATH:-data/ZINC250K/vocab.json}"
    WORKERS="${WORKERS:-30}"

    echo "=== MoleculeACE preprocess (canonical split) ==="
  ...
  *)
    ...
    echo "  moleculeace  Tensorize MoleculeACE (canonical split)"
    echo ""
    ...
    echo "  CSV_DIR    Raw CSV directory   (default depends on subcommand)"
    echo "  OUT_DIR    Output directory    (default depends on subcommand and MODE)"
    echo "  SEEDS      Space-separated list of seeds (moleculenet only)"
    echo "             (default: 1 2 3 42 43 44 123 456 789 1024)"
    echo "  WORKERS    Parallel workers    (default: 60 pretrain, 30 moleculeace)"
    exit 1
    ;;
esac
